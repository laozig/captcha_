...
脚本会自动检测当前运行环境，并选择合适的服务器地址：

- **本地环境**：使用`http://localhost:9898/`作为服务器地址
- **远程环境**：使用`http://captcha.tangyun.lat:9898/`作为服务器地址

环境检测逻辑：
1. 检查主机名是否为localhost、127.0.0.1或私有IP
2. 检查是否使用file://协议（本地文件）
3. 检查是否在Electron等本地应用中运行

如果远程服务器不可用，脚本会自动尝试切换到本地服务器。
...
脚本现在具有智能环境检测功能，会根据当前运行环境自动选择合适的OCR服务器地址。你可以在控制台查看环境检测结果：

- 环境检测：显示"本地环境"或"远程环境"
- 服务器状态：显示"在线"、"离线"、"异常"或"超时"
- 自动切换：如果远程服务器不可用，将自动尝试切换到本地服务器
...
**Q: 验证码识别失败怎么办？**  
A: 请确保OCR服务器正常运行，并检查浏览器控制台是否有错误信息。脚本会自动尝试切换服务器。

**Q: 如何修改OCR服务器地址？**  
A: 通常不需要手动修改，脚本会自动选择合适的服务器地址。如果确实需要修改，可以编辑脚本中的`detectEnvironment`函数。

**Q: 支持哪些浏览器？**  
A: 支持安装了Tampermonkey的Chrome、Firefox、Edge等主流浏览器。

**Q: 如何查看当前环境和服务器状态？**  
A: 在浏览器控制台(F12)中查看脚本输出的调试信息，包含环境检测结果和服务器连接状态。
